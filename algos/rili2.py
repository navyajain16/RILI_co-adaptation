...
from algos.TD3 import Actor,Critic 
...
class RILI1(object):
    def __init__(self, action_space, state_dim, timestep):
        ...
        self.max_action = torch.tensor(action_space.high, dtype=torch.float32).to(self.device)
        self.discount=0.99
        self.updates = 0
		#tau=0.005,
        ...
        self.actor = Actor(30+state_dim, self.action_dim, self.hidden_size,self.max_action,action_space).to(self.device)
        self.actor_target = Actor(30+state_dim, self.action_dim, self.hidden_size,self.max_action).to(self.device)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=0.001)
        
        self.critic = Critic(30+state_dim, self.action_dim, self.hidden_size).to(self.device)
        self.critic_target = Critic(30+state_dim, self.action_dim, self.hidden_size).to(self.device)
    ...
    def select_action(self, state, z):
            ...
            action = self.actor(context)
            #print(self.actor(context))
    ...
    def update_parameters(self, memory, batch_size):
        ...
        if self.updates % self.policy_freq == 0:
            ...
            return critic_loss.item(), actor_loss.item(), ae_loss.item(), curr_loss.item(), next_loss.item(), kl_loss.item()
        else:
            if self.updates % self.target_update_interval == 0:
                soft_update(self.critic_target, self.critic, self.tau)
            self.updates += 1
            return critic_loss.item(), None, ae_loss.item(), curr_loss.item(), next_loss.item(), kl_loss.item()
